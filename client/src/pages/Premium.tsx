/** Studio page — avatar/voice selection and video generation via local backend. */
    ...
    pollRef.current = setInterval(async () => {
      ...
      try {
        ...
        if (data.status === "done") {
          ...
          if (data.job_type === "repo") { navigate(`/repo/${jobId}`); return; }
          if (data.job_type === "concept_algo") { navigate(`/concept/${jobId}`); return; }
          if (data.job_type === "prompt") { navigate(`/prompt/${jobId}`); return; }
      ...
      <div className="mx-auto max-w-4xl">
        <div className="flex items-center gap-3">
          <div className="inline-flex items-center gap-2 rounded-full bg-accent/10 px-3 py-1 text-xs font-semibold text-accent">
            <Crown className="h-3.5 w-3.5" /> Studio
          </div>
        </div>
